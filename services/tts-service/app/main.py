...
def main():
    ...
    rpc_queue_name = os.environ.get("RABBITMQ_TTS_QUEUE", "tts_request_queue")
        ...
        try:
            ...
            def on_request(ch, method, props, body):
                ...
                try:
                    ...
                    print(f" [x] 收到請求: '{text_to_synthesize}'", flush=True)

                    # 使用 TTS 服務進行文字轉語音
                    object_name = tts_service.synthesize_text(text_to_synthesize)
                    response_payload = json.dumps({"object_name": object_name})
            ...
            print(f" [*] TTS Service 正在等待 RPC 請求於佇列 '{rpc_queue_name}'...", flush=True)
