...
def main():
    ...
    rpc_queue_name = os.environ.get("RABBITMQ_STT_QUEUE", "stt_request_queue")
        ...
        try:
            ...
            def on_request(ch, method, props, body):
                ...
                try:
                    ...
                    print(f" [x] 收到請求: {bucket_name}/{object_name}", flush=True)

                    # 使用 STT 服務進行語音轉文字
                    transcribed_text = stt_service.transcribe_audio(bucket_name, object_name)
                    response_payload = json.dumps({"transcribed_text": transcribed_text})
            ...
            print(f" [*] STT Service 正在等待 RPC 請求於佇列 '{rpc_queue_name}'...", flush=True)
