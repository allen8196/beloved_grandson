...
if HUGGING_FACE_TOKEN:
    ...
    try:
        ...
        print("⚠️  警告: `huggingface-cli` 指令不存在。請確保 `huggingface-hub` 套件已安裝。", flush=True)
    except subprocess.CalledProcessError as e:
        # 登入失敗，可能因為 token 無效
        print(f"⚠️  警告: Hugging Face CLI 登入失敗。Token 可能無效。錯誤: {e.stderr}", flush=True)
else:
...
class OptimizedOrpheusTTS:
    ...
    def _decode_and_redistribute(self, generated_ids_batch: torch.Tensor) -> List[torch.Tensor]:
        ...
        for row in generated_ids_batch:
            ...
            last_occurrence_idx = token_indices[-1].item() if len(token_indices) > 0 else -1
        ...
        for code_list in code_lists:
            ...
            codes = [torch.tensor(layer, dtype=torch.int32).unsqueeze(0) for layer in [layer_1, layer_2, layer_3]]
            with torch.no_grad():
