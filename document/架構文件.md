...
flowchart TD
    ...
    WebApp -- "6a.寫入對話紀錄" --> PostgreSQL
    WebApp -- "6b.將結果依 task_id 存入" --> Redis
    User -- "7.定期查詢 task_id" --> WebApp
    WebApp -- "8.依 task_id 讀取結果" --> Redis
...
*   **Web App (Flask)**: 系統的主要入口與**狀態協調器**。其職責包括：
    ...
    *   接收 AI 任務請求，將其發布到 `RabbitMQ` 的 `tasks.queue`。
    *   提供客戶端查詢任務結果的 API (從 Redis 讀取)。
    *   **(新)** 訂閱 `RabbitMQ` 的 `results.queue`，接收 AI Worker 的運算結果。
    *   **(新)** 負責將接收到的結果進行**原子化持久儲存**：更新 `Redis` 中的任務狀態，並將完整的對話紀錄寫入 `PostgreSQL`。
*   **AI Worker**: 非同步 AI 運算單元。其職責被嚴格限定在執行計算密集型的 AI 任務。它持續監聽 `RabbitMQ` 的 `tasks.queue` 佇列。收到任務後，會依序執行以下操作：
    1.  呼叫 `STT Service` 將語音轉為文字。
    2.  呼叫 `LLM Service` 進行語言模型處理與 RAG 檢索。
    3.  呼叫 `TTS Service` 將模型的文字回覆轉為語音。
    4.  將包含所有結果（如 `text_response`, `audio_output_url`）的**單一事件**發布到 `RabbitMQ` 的 `results.queue` 佇列，供後續處理。
...
*   **Milvus**: 專為 AI 應用設計的向量資料庫，用於儲存 RAG 所需的向量化知識數據。
    ...
    *   **MinIO**: 為 Milvus 提供底層的物件儲存，用於存放向量索引等檔案。
...
### 4.1. 非同步 AI 任務處理與持久化流程 (優化後)
1.  **提交任務**: 使用者透過 `Web App` 提交 AI 請求。`Web App` 生成 `task_id`，並將任務發布到 `RabbitMQ` 的 `tasks.queue`。
2.  **派發與處理**: `RabbitMQ` 將任務推送給 `AI Worker`。`AI Worker` 專注執行 STT, LLM, TTS 等 AI 運算。
3.  **發布結果**: `AI Worker` 完成運算後，將包含完整結果的**單一事件**發布到 `RabbitMQ` 的 `results.queue`。
4.  **訂閱與持久化**: `Web App` 訂閱 `results.queue`。收到結果後，它會執行**原子性的資料庫操作**：將完整的對話紀錄 (`conversations`, `messages`) 寫入 `PostgreSQL`，並更新 `Redis` 中的任務狀態與結果。
5.  **查詢結果**: 在此期間，使用者端可持續使用 `task_id` 向 `Web App` 輪詢查詢任務狀態。
6.  **回傳結果**: 一旦 `Web App` 更新了 `Redis`，輪詢請求便能讀取到最終結果並回傳給使用者。
