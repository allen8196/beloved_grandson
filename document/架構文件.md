...
*   **Web App (Flask)**: 系統的主要入口與**狀態協調器**。其職責包括：
    ...
    *   **(資料一致性)** 確保 API 的請求/回應（如 `full_name` 欄位）與後端資料庫結構定義清晰對應，維持數據流的透明與一致性。
    *   **(API 文件化)** 提供即時、互動式的 API 文件。透過整合 Swagger，`Web App` 在 `/apidocs` 端點上公開了所有 API 的規格與測試工具，使其成為 API 消費者（如前端開發者）的**單一事實來源 (Single Source of Truth)**。
