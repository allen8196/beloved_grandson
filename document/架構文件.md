...
*   **Web App (Flask)**: 系統的主要入口，負責處理前端使用者的 HTTP 請求。其職責包括：
    *   提供 API 端點。
    *   處理使用者身份驗證與資料管理。
    *   接收 AI 任務請求，並將其封裝成任務訊息發布到 `RabbitMQ`。
    *   提供客戶端查詢任務結果的 API。
*   **AI Worker**: 非同步任務處理器。它獨立於 `Web App` 運行，持續監聽 `RabbitMQ` 中的任務。收到任務後，會依序執行以下操作：
    1.  呼叫 `STT Service` 將語音轉為文字。
    2.  呼叫 `LLM Service` 進行語言模型處理與 RAG 檢索。
    3.  呼叫 `TTS Service` 將模型的文字回覆轉為語音。
    4.  將最終處理結果以 `task_id` 為鍵，存入 `Redis`。
    5.  在過程中可能會將對話紀錄寫入 `PostgreSQL`。
...
*   **RabbitMQ**: 高效能的訊息佇列，作為 `Web App` 與 `AI Worker` 之間的中介，實現了請求提交與任務處理的解耦與非同步化。
...
### 4.1. 非同步 AI 任務處理流程
1.  **提交任務**: 使用者透過 `Web App` 提交一個 AI 請求 (例如上傳音檔)。`Web App` 生成一個唯一的 `task_id`，並將任務內容與 `task_id` 一同發布到 `RabbitMQ`。
2.  **派發任務**: `RabbitMQ` 將任務推送給一個已訂閱的 `AI Worker`。
3.  **處理任務**: `AI Worker` 依序呼叫 STT, LLM, TTS 服務完成 AI 處理。
4.  **儲存結果**: `AI Worker` 將最終結果以 `task_id` 為鍵存入 `Redis`。
5.  **查詢結果**: 使用者端透過輪詢 (Polling) 或其他前端通知機制，使用 `task_id` 向 `Web App` 查詢結果。
6.  **回傳結果**: `Web App` 從 `Redis` 中讀取到結果後，回傳給使用者。

### 4.2. RAG 檢索流程
1.  `AI Worker` 呼叫 `LLM Service`。
2.  `LLM Service` 在處理請求時，向 `Milvus` 發出查詢，檢索最相關的向量化知識。
3.  `Milvus` 返回檢索結果。
4.  `LLM Service` 結合原始問題與檢索到的知識，生成更精確、更豐富的回答。
