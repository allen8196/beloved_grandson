...
### 4.1. 音檔儲存與非同步 AI 任務處理流程 (MinIO 整合)
1.  **上傳與存入 MinIO**: 使用者透過客戶端上傳音檔。請求到達 `Web App`。`Web App` 立即將音檔串流至 `MinIO` 的 `audio-inputs` 儲存桶 (bucket) 中，並取得該物件的內部存取路徑 (e.g., `minio:9000/audio-inputs/some-file.wav`)。
2.  **提交任務**: `Web App` 生成 `task_id`，並將包含 `MinIO` 物件路徑的任務訊息發布到 `RabbitMQ` 的 `tasks.queue`。
3.  **派發與處理**: `RabbitMQ` 將任務推送給 `AI Worker`。`AI Worker` 從訊息中解析出 `MinIO` 路徑，從 `MinIO` 下載音檔進行 STT, LLM, TTS 等 AI 運算。
4.  **存回 MinIO**: `AI Worker` 將 TTS 生成的音檔存回 `MinIO` 的 `audio-outputs` 儲存桶。
5.  **發布結果**: `AI Worker` 完成運算後，將包含 `MinIO` 輸出音檔路徑的完整結果發布到 `RabbitMQ` 的 `results.queue`。
6.  **訂閱與持久化**: `Web App` 訂閱 `results.queue`。收到結果後，它會執行**原子性的資料庫操作**：將包含 `MinIO` 路徑的完整對話紀錄寫入 `PostgreSQL`，並更新 `Redis` 中的任務狀態與結果。
7.  **查詢結果**: 在此期間，使用者端可持續使用 `task_id` 向 `Web App` 輪詢查詢任務狀態。
8.  **回傳結果**: 一旦 `Web App` 更新了 `Redis`，輪詢請求便能讀取到最終結果。結果中包含的音檔 URL 是經過 `Nginx` 代理的安全外部連結 (e.g., `https://your-domain.com/audio/some-file.mp3`)，而非 `MinIO` 的內部路徑。
